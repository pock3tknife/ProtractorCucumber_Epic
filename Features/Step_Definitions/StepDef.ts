...
When(
  "I give the customer details {string}, {string},{string}",
  { timeout: 60 * 1000 },
  async function (fname, lname, postcode) {
...
When(
  "I click on Add customer button",
  { timeout: 60 * 1000 },
  async function () {
  ...
  try {
    await browser
      ...
      .then(function (popupmessage) {
        ...
        expect(popupmessage).to.include(
